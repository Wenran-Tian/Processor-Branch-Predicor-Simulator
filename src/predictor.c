...
// we can just use the compressed data version of gsahre 13
...
//	uint32_t threshold_h;
//	uint32_t threshold_p;     
//
//	uint8_t *Predictor_Chooser; 
//} CU;
...
init_predictor()
{
  ...
  //
	...
	if(bpType == CUSTOM){
		...
		GP.BHT = (uint8_t*)  malloc(sizeof(uint8_t)*(length/4));
		for(i=0;i<(length/4);i++) GP.BHT[i] = 85; // 01010101
...
//  	CU.h_bits = 14;
//  	CU.history_queue = 0;
//  	length = (int) pow(2, CU.h_bits) ;
//  	CU.threshold_h = (int) pow(2, CU.h_bits) - 1;
//  	CU.threshold_p = (int) pow(2, CU.p_bits) - 1; 
//  	for(i=0;i<16384;i++) CU.Prediction[i] = 1;    // 0101
...
//		return TAKEN;
    ...
    case CUSTOM:{
//    	pc = pc & CU.threshold_p;
//    	val1 = pc & ((int)pow(2,CU.p_bits - CU.h_bits)-1);
//    	index = (pc&CU.threshold_h)^CU.history_queue;
//    	index = index&CU.threshold_h;
//    	index += (val1 << (CU.p_bits - CU.h_bits));
//    	return CU.Prediction[index]>1;
...
train_predictor(uint32_t pc, uint8_t outcome)
{
  ...
  //
  uint32_t operand, buff,buff1, index,r1,r2,val1,val2;
    switch (bpType) {
    case STATIC:
      return;
    case GSHARE:{
...
//    	return;
		...
		val1 = index /4;
		val2 = index%4;
//		printf("%d  ",GP.BHT[val1]);
    	operand = (GP.BHT[val1] >> 2*val2) && 3;
    	operand = (outcome==TAKEN ? operand+1 :  operand-1);
		operand = rangeRestrict(operand);
		switch(val2){
			case 0:  GP.BHT[val1] = (GP.BHT[val1]&252) + (operand << 2*val2); break;
			case 1:  GP.BHT[val1] = (GP.BHT[val1]&243) + (operand << 2*val2); break;
			case 2:  GP.BHT[val1] = (GP.BHT[val1]&207) + (operand << 2*val2); break;
			case 3:  GP.BHT[val1] = (GP.BHT[val1]&63) + (operand << 2*val2); break;
		}
